  ...
  describe '購入情報の保存' do
    ...
    context "購入情報を登録できない場合" do
      ...
      it 'phone_numberが全角だと登録できないこと' do
        ...
        @buyer_receiver.valid?
        expect(@buyer_receiver.errors.full_messages).to include('Phone number input 11 half-width numbers')
      end
      ...
      it 'phone_numberは11桁以内でないと登録できないこと' do
        @buyer_receiver.phone_number = '090123456789'
        @buyer_receiver.valid?
        expect(@buyer_receiver.errors.full_messages).to include('Phone number input 11 half-width numbers')
      end
    end
